...
async def temp_message(info_field, text, duration = 5):
...
def add_media(arg=None):
    ...
    # Info bar (shows temporary messages)
    info_field = TextArea(height=2, style="class:output", read_only=True)

    async def show_temp_message(target_field, text, duration=5):
        """Display `text` for `duration` seconds, then clear. Non-blocking."""
        target_field.text = text
        get_app().invalidate()
        await asyncio.sleep(duration)
        target_field.text = ""
        get_app().invalidate()

    # Button handler for the star: show a 5-second status tip
    def star_handler():
        get_app().create_background_task(
            show_temp_message(info_field,
                              "Status help: e.g., planned / ongoing / completed / dropped.",
                              5)
        )

    star_button = Button(text=" * ", handler=star_handler)
    ...
    def on_cancel():
        ...
        for i in range(2):  # Repeat animation 2 times
            for frame in frames:
                print(f"\r{frame}", end='', flush=True)
                time.sleep(0.2)
    ...
    layout = HSplit([
        ...
        VSplit([Label("status: ", width=10), status_field, star_button]),  # <-- star button added
        ...
        result_field,
        info_field,  # <-- info bar for temporary messages
    ])

    app = Application(layout=Layout(layout), key_bindings=kb, full_screen=True, mouse_support=True)
    result = app.run()
